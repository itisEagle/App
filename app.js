                ...
                prodContainer.innerHTML += `
                    <div class="product-card" onclick="window.location.href='product.html?id=${id}'">
                        ...
                        <div class="p-details">
                            ...
                            <div class="p-price">From $${displayPrice}</div>
                        </div>
                    </div>`;
            }
        });
    ...
    onValue(ref(db, 'products/' + pid), (snapshot) => {
        ...
        if(p.variants && p.variants.length > 0) {
            ...
            p.variants.forEach((v, index) => {
                variantSelect.innerHTML += `<option value="${index}" data-price="${v.price}">${v.name} - $${v.price}</option>`;
            });
            // Init with first variant
            selectedPrice = p.variants[0].price;
            selectedVariantName = p.variants[0].name;
            document.getElementById('p-price').textContent = `$${selectedPrice}`;
        } else {
